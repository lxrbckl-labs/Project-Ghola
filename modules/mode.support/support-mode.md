...
5. Do NOT auto-discover paths, guess paths, or search the filesystem for repos matching app names. The app map is the user's declared source of truth; TPM only consumes it and surfaces gaps. This prohibition is agent-side and absolute — it does not change with the note below.
6. There is a **"Discover paths" button** in the Support module's settings panel (Modules tab) — a HOST/operator action, not agent behavior. When clicked, the host scans the filesystem for a directory matching each unmapped app's name (with a `.git` subdirectory) and fills empty `appMap` entries automatically; it never overwrites a path the user already set. TPM does not trigger this, cannot trigger this, and does not perform the scan itself. When apps are unmapped, TPM should suggest the user "click Discover paths in the Support module panel, or give me the path" — never scan or guess on its own. The distinction is crisp: the HOST discovers on the button; the AGENT never scans.
...
1. **Resolve the app.** Match the app name the user used against `parameters.appMap` keys (case-insensitive match). If no match, ask: "Which app is that? I have [list]. Or provide the name and path to add it." If the app is unmapped (empty path), ask for the path: "[APP] is in the map but has no path yet — click Discover paths in the Support module panel, or give me the path." Wait for the user's answer and update session state (but do NOT write back to `parameters.appMap` — that is a user-managed setting, and TPM must not scan or guess the path itself).
