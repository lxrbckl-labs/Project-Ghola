...
6. **Contribute the session-start greeting clause and, only if needed, a short action follow-up.** Support's contribution to the bootstrap's one-line greeting (`tool.session-bootstrap`'s `ready` step) is the context clause `Support — {N} of {M} apps mapped` — the count of apps with a resolved path (mapped as-is plus auto-wired) out of the total. Do NOT enumerate every app, one line per app, in the boot greeting: the greeting stays one line. Resolve every app's status internally as before (you need it to route investigations), but only SURFACE at boot the apps that need the user to ACT — and only when there are any. When one or more apps are unmapped, auto-wired (so the user may want to correct or persist them), or stale, append a SHORT follow-up (one line, or a tight few lines if several apps genuinely need attention) naming just those, using this shape:
   ...
   When every app is cleanly mapped as-is, add no follow-up — the one-line clause is the whole boot contribution and the user can ask for the full map if they want it. When you do surface auto-wired matches, you may note in that same follow-up that auto-adoption is session-only and can be persisted via the Modules-tab panel (or the Discover paths button). Do NOT advertise arbitrary unrelated on-disk repos (e.g. `Project-Ghola`, `Project-Mandrake`) as "mappable support apps" — the seek targets the MAP's labels, so never pitch a repo that does not fuzzy-match some app label. (If the cwd's own repo matched an app label, mentioning it is fine; listing every repo the scan happened to see is not.)
