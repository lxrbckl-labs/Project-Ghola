...
2. **Run ONE bounded, read-only filesystem scan** that does double duty for the whole map — existence-checks the already-mapped paths AND gathers candidate repos for the unmapped apps, in a single bash call rather than a walk per app. Scope the scan to the WSL-native project root (the immediate subdirectories of `$HOME/projects/*`) plus the current terminal cwd's own repo, and restrict candidates to directories that are git repositories (contain a `.git`, or `git rev-parse` succeeds). Do NOT scan the whole filesystem, and do NOT do a slow `/mnt/c` walk. Use the results two ways: for each mapped app, if its configured path does not exist on disk, demote it to a third category — **stale** (path configured but not found on disk); for each unmapped app, hold the git-repo candidates for the fuzzy match in step 5.
3. **Auto-discover and auto-adopt a path for each unmapped app (session-scoped).** For each unmapped app, fuzzy-match its label against the candidate repos: normalize both the app label and each candidate repo's basename to lowercase, strip trailing digits and a trailing `-api`/`-web`/`-client` suffix from the basename (so `cmms0`, `cmms-api`, and `cmms` all normalize to `cmms`), then match when the normalized basename equals or starts-with the normalized label. Example: label `CMMS` matches `~/projects/cmms0`. If one or more candidates match, **pick a single best default and auto-adopt it** as that app's active path for this session: prefer the repo that is the current terminal cwd's own repo when it matches the label; otherwise take the lowest-numbered / first match. Auto-adoption wires the path in SESSION state only — TPM does NOT write it back to `parameters.appMap`. If several clones matched (e.g. `cmms0`..`cmms6`), remember that alternates exist so you can name them when reporting. If NO candidate matches an unmapped app in the bounded scope, leave it unmapped and fall back to asking (step 6 / investigation flow).
4. If ALL apps are unmapped and NONE could be auto-discovered, surface that the mode is effectively idle until the user provides at least one path: "All apps in the map are unmapped and none matched a local repo — provide a path for any app and I'll be ready to investigate."
5. Compose the session-start announcement into a single coherent opening message. State N of M apps mapped with their paths. For apps auto-wired in step 3, report exactly what was picked and make clear it is correctable and session-only, e.g. `Auto-wired for this session: CMMS -> ~/projects/cmms0 (say "CMMS is at <path>" to correct)`; when several clones matched, name the chosen one and note that alternates exist. For apps with NO match in the bounded scope, report them unmapped and invite either a direct path or the host "Discover paths" button: "[unmapped apps] not mapped and no local match found — give me the path, or click Discover paths in the Support module panel." Include stale entries with a note: "[app] path configured but not found on disk." Since auto-adoption is session-only, suggest the user persist any good match via the Modules-tab panel (or Discover paths) so it survives into future sessions.
6. The agent's session-time auto-adopt (steps 2-3) and the host's **"Discover paths" button** are complementary, not exclusive — both can find paths, they differ in scope and persistence. The Discover paths button lives in the Support module's settings panel (Modules tab) and is a HOST/operator action: when clicked, the host scans the filesystem for a directory matching each unmapped app's name (with a `.git` subdirectory), fills empty `appMap` entries, and **persists** them into settings (it never overwrites a path the user already set). That is the PERSISTENCE mechanism — an exact-name scan written back to the map. The agent's auto-adopt is the LIVE, SESSION-SCOPED counterpart: it runs a fuzzy match at session start, adopts a best-guess path for the current session only, and writes nothing back. Exact-name host discovery can miss real repos whose on-disk name differs from the app label (app `CMMS` living on disk as `cmms0`); the agent's fuzzy reasoning bridges that gap for the session. When an app stays unmapped after auto-adopt, TPM invites the user to give a path directly or click Discover paths to wire it persistently.
...
1. **Resolve the app.** Match the app name the user used against `parameters.appMap` keys (case-insensitive match). If no match, ask: "Which app is that? I have [list]. Or provide the name and path to add it." If the app was unmapped in `parameters.appMap`, it should usually already be auto-wired from the session-start auto-discovery (App resolution steps 2-3) — use that session path and note it was auto-wired ("[APP] is auto-wired to `<path>` for this session — say so if that's wrong"). Only if no local match was found at session start (the app is still unmapped) ask for the path: "[APP] is in the map but has no path yet, and no local repo matched — click Discover paths in the Support module panel, or give me the path." Wait for the user's answer and update session state (but do NOT write back to `parameters.appMap` — that is a user-managed setting; the agent's auto-adopt is session-only).
