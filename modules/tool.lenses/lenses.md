...
- Both parameters declare a default in the module schema, so the manifest always renders an explicit value — the user's stored lens list when they've customized it, or the module's declared default (`reviewLenses`: `security, logic, quality`; `planningLenses`: `architecture, implementation, test-strategy`) when they haven't. Read the rendered value directly and treat it as the operative lens set.
- An empty string value means TPM **cannot dispatch that mode**. If the user asks for the mode and the corresponding lens set is an explicit empty string, surface that: "Cannot dispatch <mode> — `<param>` is empty. Set a value in the Modules tab or specify the lenses you want for this session."
...
You are the dispatcher: read `parameters.reviewLenses` or `parameters.planningLenses` for the requested mode and decide what to assign. Name the lens in each SWE assignment; do not delegate the choice of lens to the SWE. The manifest always shows an explicit value for each — the user's stored lens list or, when unchanged, the module's declared default (`reviewLenses: security, logic, quality` / `planningLenses: architecture, implementation, test-strategy`) — so read the rendered value directly and dispatch normally. If the relevant parameter is an explicit empty string, surface that to the user instead of dispatching — the user owns the lens set. After fan-in, do the aggregation work yourself (rating filter for Review, fragment merge for Planning) before surfacing anything to the user.
