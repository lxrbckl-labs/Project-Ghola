...
4. Write a commit message that fills the format template provided in your launch message. Substitute the placeholders from the diff: <TICKET> = the active ticket id if one is evident, substituted bare (otherwise drop the entire <TICKET>: prefix — the ticket id, the colon, AND the trailing space — cleanly, so the message is just the summary and never starts with a leading colon or space), <summary> = a concise one-line description. Add a short body describing what changed and why if it adds value.
