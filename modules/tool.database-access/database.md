...
In the `not-installed` and `error` states the panel shows two action buttons: "Copy install instructions" (copies a configurable prompt to the clipboard) and "Configure path…" (opens the `ghola.linqpadConnectionsPath` VS Code setting so the user can supply an explicit path to the XML file). The user must resolve the underlying problem before the dropdown will populate. In the `loading` state no action buttons are shown.
