...
When this module is loaded, the session has a single project-wide convention for handling credentials: agents NEVER read secrets directly. Secrets live in wrapper scripts that source the credential at call time and never expose the value in their output. The rule was previously inlined in the cores' hard rules ("NEVER read or echo secrets" — covering `*_TOKEN`, `*_SECRET`, `*_KEY`, `*_PASSWORD` env vars and the deployment secrets file); this module promotes it to a single configurable policy and adds an approved-wrappers registry. Every agent reads this same fragment; TPM orchestrates wrapper invocation, SWE invokes the wrappers, and QA verifies no secrets were read or echoed.
...
The core prompts (TPM, SWE, QA — hardcoded in `prompts/cores/`) inline a "NEVER read or echo secrets" hard rule that covers `*_TOKEN`, `*_SECRET`, `*_KEY`, `*_PASSWORD` env vars and the deployment secrets file. With this module loaded:
...
- **Module disabled** (no `tool.secrets-wrapper-pattern` in the Session Manifest): the cores' inline hard rules act as fallback. Agents still refuse to read or echo `*_TOKEN`, `*_SECRET`, `*_KEY`, `*_PASSWORD` env vars and the deployment secrets file, but the pattern set is fixed, the wrapper registry is unavailable, and the violation behavior is hardcoded refuse.
