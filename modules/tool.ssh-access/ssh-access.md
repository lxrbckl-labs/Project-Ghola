...
- **Enforce the `parameters.timeoutSeconds` wall-clock bound on every SSH command.** No remote command may run unbounded, or a hung SSH session (a connect that never completes, a command that blocks on input, a remote process that never returns) would hang the whole session. Cap each invocation at `parameters.timeoutSeconds` (default 30): wrap the command with `timeout <seconds> ssh <host> '<command>'` (or the equivalent tool-level timeout), and additionally set an SSH connect timeout such as `-o ConnectTimeout=<seconds>` so a stalled connection also aborts. If a command exceeds the bound, abort it and report the timeout to TPM (naming the host, the command, and the elapsed limit) rather than waiting; do not silently retry. A value of 0, if the user sets one, means no wall-clock cap, but the default and the recommendation is a finite bound so no single SSH call can stall the session.
