...
**What bounds the capture file.** Three limits apply to every write, so the classify stage is never surprised by any of them: the file undergoes **whole-file rotation at 5 MB** — the write that would cross the bound renames the current file to `<file>.1` (replacing any prior `.1` generation) and starts a fresh empty file, bounding total disk use at roughly 2x the limit; each record's `body` is **clamped to 8000 characters**, with an inline `…[body truncated by Ghola capture]` marker appended when a body is cut, so a truncated record is never mistaken for a genuinely short one; and every body passes through **unconditional, best-effort secret redaction before it is written** — any `[redacted-...]` marker you see is Ghola's own insertion, not text the reviewer wrote, and the redaction is a mitigation against a pasted credential, not a guarantee that none survive.

**The capture path applies redaction and clamping BEFORE writing, so the `body` you read from the file is not necessarily the body as it reads on Bitbucket.** A redacted secret or a truncation marker is Ghola's addition to the record, not part of the original comment — keep that distinction in mind at the classify stage, where the rule this comment asserts should be drawn from the reviewer's actual words, not from a marker Ghola inserted around them.
...
1. **An existing on-disk home wins.** A home matches when its **filename** OR its **frontmatter `project:`** matches ANY of {the canonical key, the cwd basename, a configured `projectNicknames` token}, case-insensitive. This is a single symmetric OR-test — not a priority order between the two fields. `tool.obsidian-notes`' own worked example (a directory renamed to `Project-Ghola` whose remote still reads a stale `Project-Legacy`) resolves via **cwd-basename affinity on the filename**: `Projects/Project-Ghola.md` matches the cwd basename `Project-Ghola` even though the canonical key derived from the stale remote does not. Checking frontmatter before filename (or the reverse) finds the same match either way, because both are tested against the same three candidates — do not build in an ordering that isn't there.
2. **Only if nothing matches** does the canonical key name a new home; failing that, the lowercase cwd basename.
3. **Two candidates means ASK.** Never guess, never merge, never pick the newer one. TPM asks the operator which home is the home.
