...
The probe also reports `now=` — the current date and time — so the boot phase needs no separate `date` command. Feed `now` into the orientation paragraph and any date reasoning (see the mapping table's `now` → `ready` row).
...
- **Wave 1 — resolve prerequisites.** A single `echo` that resolves the paths later steps need as concrete arguments: `$GHOLA_ROOT` (for the probe location and for the module `.md` paths) and `$GHOLA_TPM_PROMPT_FILE` if not already resolved. This wave must precede Wave 2 because everything downstream needs `$GHOLA_ROOT`. Do NOT additionally echo values the probe already reports (env state, team, version, branch, ticket, etc.) — the probe is the single source for those, and re-echoing them is duplicate work, not extra context.
...
- **Wave 3 — dependent follow-ups in parallel.** In ONE message, run the steps that need Wave 2's output: the `detail_file` read (its path comes from the probe digest; its content feeds the ticket body for the orientation paragraph and the handoff block for resume) plus any mode-specific post-probe check a delegated module requires (e.g. a War Mode ledger board lookup keyed by the probe's ticket subject). Batch these together — they depend only on the probe's output, not on each other.
