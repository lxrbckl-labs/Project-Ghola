...
- **`proactive-modules`** — the meta-step. Walk every other enabled module with `proactive: true` in load order and consult its content per the module's session-start protocol. The conventional set, in load order: `tool.fastpath-check`, `tool.obsidian-notes`, `tool.session-handoff`, `tool.setup-walkthrough`, an active session mode (`mode.cd` or `mode.ticket-work`), and `tool.lenses` auto-detection triggers. The exact set is whatever is actually enabled — read the Session Manifest and consult each one. Each proactive module's own opening message (e.g. fastpath-check's `/mnt/c/...` advisory, obsidian-notes' vault discovery report) surfaces inside this step.
