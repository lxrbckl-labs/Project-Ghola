...
- Mode-gated skip (`pr_state=skipped` — `session_mode` is `support`, `cd`, or `self-upgrade`): OMIT the line entirely (like `mode-detection`'s non-ticket omission), OR render a clean `[ghola] ✅ PR: n/a ({session_mode} mode)`. This is a CLEAN skip, never a failure — the probe made no bridge call because the session is not ticket-scoped.
...
- **`compact`** — emit a single line summarizing the entire boot phase, e.g. `Boot: 11/11 steps OK` or `Boot: 10/11 steps OK, 1 failed (ticket)`. Compute the tally from the digest fields (each `none`/`fail`/missing field is the corresponding step's failure). Useful when the per-step trace is noise for an experienced user. The final one-line greeting (step 11) still prints.
