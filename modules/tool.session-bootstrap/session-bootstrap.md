...
| 6. `ticket` | `ticket_key`, `ticket_state`, `ticket_status`, `ticket_summary`, `session_mode` | `[ghola] ✅ Ticket: {KEY} (pulled from Jira)` / `❌ Ticket: {KEY} (Jira unavailable ...)` / `⏭️ Ticket: none detected from branch` / `⏭️ Ticket: n/a ({mode} mode — not ticket-scoped)` when `ticket_state=skipped` (in a self-upgrade session: `⏭️ Ticket: n/a (self-upgrade — not ticket-scoped)`) |
| 7. `related-pr` | `pr_state`, `pr_id`, `pr_title`, `pr_url`, `pr_author`, `session_mode` | `[ghola] ✅ PR: #{pr_id} (OPEN) — {pr_title}` when `pr_state=OPEN`; `✅ PR: #{pr_id} ({pr_state}) — {pr_title}` for a found-but-closed PR (`MERGED`/`DECLINED`/other closed state); `✅ PR: none open for this branch` when `pr_state=none`; `❌ PR: (Bitbucket unavailable)` when `pr_state=na` (degraded, never blocks); OMITTED (or `⏭️ PR: n/a ({mode} mode)`) when `pr_state=skipped` in a non-ticket mode |
| 8. `notes` | `vault`, `notes_file`, `notes_exists`, `session_mode` | `[ghola] ✅ Notes: {Project}/{Number}.md resuming ...` / `... created` (see the notes step's note on who creates) / clean skip when no vault or no ticket / `⏭️ Notes: ({session_mode} mode — surface owned by mode.{session_mode})` when `ticket_state=skipped` (in a self-upgrade session: `⏭️ Notes: (self-upgrade — not ticket-scoped)`) |
| 9. `mode-detection` | `pr_author`, `mode`, `base`, `ahead`, `session_mode` | `[ghola] ✅ Dev mode: planning \| review \| author` — an author-vs-review trace, meaningful only in ticket-scoped sessions. **PRIMARY signal is identity**: when `pr_author` is present, compare it (case-insensitive, trimmed) against the operator's `bitbucketUsername` from `tool.operator-profile` — unequal -> `review`, equal -> `author`. FALLBACK to the probe's git-based `mode` when there is no PR or no `bitbucketUsername`. Labeled `Dev mode:` (not `Mode:`) to disambiguate from the banner's `Mode` row. **OMITTED entirely** (no line, no skip placeholder) when `session_mode` is `support`, `cd`, or `self-upgrade`; rendered as usual for `ticket-work`, `unconstrained`, and War Mode / Sardaukar sessions |
| 10. `resume` | `handoff_date` (+ `detail_file` for the block) | `[ghola] ✅ Resume: {handoff_date} handoff found` / `✅ Resume: fresh session` |
...
- **`related-pr` — meaning owned by `integration.bitbucket-pr-comments` / `integration.atlassian-suite`.** The probe resolves the branch's PR through the same `bb-bridge.mjs find-pr` path those integrations expose (best-effort, `timeout 3`, degrading to `pr_state=na`); those modules remain authoritative for what a PR record means and for any PR writes. This module reads them for meaning and renders the probe's `pr_*` fields. The step also feeds `pr_author` to `mode-detection` as its primary review-vs-author signal.
...
- **`mode-detection` — review-vs-author identity from `tool.operator-profile`.** The probe surfaces `pr_author` (via `related-pr`) and the git-based `mode`; the review-vs-author decision compares `pr_author` against `tool.operator-profile`'s `bitbucketUsername` (identity-primary), falling back to the git `mode` when no PR/handle is available. This module reads `tool.operator-profile` for the operator handle and renders the resolved dev-mode.
...
- `mode.ticket-work` not enabled: `[ghola] ⏭️ Ticket: (ticket-work module off)` — the probe may still carry `ticket_*` fields, but with the module off, report it disabled.

### 7. `related-pr`

Read `pr_state`, and (when present) `pr_id`/`pr_title`/`pr_url`/`pr_author`, plus `session_mode`. For a ticket-scoped session with a resolvable branch the probe called `bb-bridge.mjs find-pr` (the SAME bridge path the ticket pull uses) with a TIGHT `timeout 3` so boot never drags; the meaning of a PR record is owned by `integration.bitbucket-pr-comments` / `integration.atlassian-suite` (this step reads them for meaning and renders the probe's `pr_*` fields). The call is best-effort and never blocks: any failure degrades to `pr_state=na`. This step also supplies `pr_author` to the `mode-detection` step below as the primary review-vs-author signal.
...
- Bitbucket unavailable (`pr_state=na` — bridge unreachable, timed out, integration off, or garbage output): `[ghola] ❌ PR: (Bitbucket unavailable)` — a degraded line with a one-clause reason; NEVER blocks, continue to the next step.
...
1. **PRIMARY — identity comparison.** When `pr_author` is present (an open or found PR was resolved) AND `tool.operator-profile` is loaded with a non-empty `bitbucketUsername` parameter (the TPM has this value in its composed prompt when that module is loaded), compare `pr_author` against `bitbucketUsername` **case-insensitively, after trimming surrounding whitespace**. If they are **not equal**, the mode is `review` (you are reviewing someone else's PR); if they are **equal**, the mode is `author`.
2. **FALLBACK — the probe's git-based `mode`.** Use the probe's `mode` field exactly as today when there is no PR (`pr_state` is `none`/`na`/`skipped`, so no `pr_author`), OR when `tool.operator-profile` / `bitbucketUsername` is absent or empty. This preserves the existing behavior wherever the identity signal is unavailable.
...
- **`mode.ticket-work`, `integration.bitbucket-pr-comments` / `integration.atlassian-suite`, `tool.obsidian-notes`, `tool.session-handoff`, `tool.lenses`, `tool.operator-profile`, `tool.core-allocation`** — the domain owners this module delegates to for MEANING (steps 3, 6, 8, 9, 10). The consolidated probe performs the read-only gathering for these steps in one shot, but their content remains authoritative for WHAT each field means and for any WRITES (notes-file creation, lens dispatch) that happen AFTER the probe. The `related-pr` step reads the Bitbucket integrations for PR meaning; the `mode-detection` step reads `tool.operator-profile` for the operator's `bitbucketUsername` (the identity-based review-vs-author signal). This module decides WHEN (in which step) and HOW the probe's result is reported in the boot diagnostics. If a delegated module changes its internal behavior, no change here is needed — the step reads it for meaning and renders the probe's field. If a delegated module is absent, the step degrades per its per-step section.
