...
- **Well-known tools only.** Install reputable, widely-used tooling from the platform's standard package manager. NEVER fetch-and-run arbitrary scripts from unvetted URLs to install (no `curl <unknown-url> | bash` and the like). If a tool is only installable that way from an unvetted source, do not install it — surface it instead.
