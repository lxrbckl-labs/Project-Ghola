...
Step 7 above covers session START: `tool.session-handoff` surfaces a summary of the most-recent `## Session Handoff` block in the opening message, and that summary is sufficient for the opening message alone. It is not sufficient for dispatch. Before TPM's FIRST SWE or QA assignment for a given ticket in a resumed session — a ticket whose per-ticket notes file already existed before that ticket's first dispatch — TPM reads the FULL per-ticket notes file for that ticket: Ticket Summary, Implementation Notes, Changes Made, Edge Cases, Testing Procedures, and QA Findings, not just the handoff block. Dispatching an assignment off the handoff summary alone means working from a summary of a summary.
...
- **Fresh tickets are exempt.** When step 6 above just created the notes file for that ticket this session, TPM already holds its content — it wrote it. This precondition applies only on resume: when a ticket's notes file existed BEFORE that ticket's first dispatch this session.
...
In a queued multi-ticket run, this precondition applies per ticket, not once for the whole session: each ticket's full notes file is read once, before that ticket's own first dispatch. Reading ticket A's notes at the top of the session does not satisfy this precondition for ticket B — skipping the per-ticket read means ticket B (and every ticket after it) gets dispatched with zero notes context.

This full read happens once per ticket, before that ticket's first dispatch — it does not repeat before every subsequent dispatch of that same ticket in that same session.
...
**Operator-requested rebind is not a conversational cross-ticket reference.** When the operator explicitly authorizes moving the binding itself — for example, a queued multi-ticket run under `mode.ticket-pr` advancing to its next ticket — the active ticket and its notes target re-bind to the new ticket; none of the three responses above apply, and this is distinct from a user merely saying "switching" as described under `ask`. This carve-out requires `crossTicketStrictness` not be `hard-block`: under `hard-block` the sentry refuses the rebind/batch outright and names `crossTicketStrictness` as the setting blocking it. A conversational reference to another ticket is unaffected by this carve-out and still resolves exactly per the bullets above.
...
TPM pulls comments when the description alone is unlikely to answer the question at hand — most commonly: the operator asks what was said on the ticket, or TPM is looking for testing/verification detail a reviewer left as a comment rather than in the description (see "Where Testing/Verification Procedures Live" below). This capability needs no module beyond `integration.atlassian-suite`, and no setting on it: reading comments has never been gated by the comment-write capability and still is not. That suite's `enableJiraCommentWrite` setting gates POSTING a comment only — it is the gate on the one Jira write, not on this read, so `get-comments` works the same whether it is on or off.
