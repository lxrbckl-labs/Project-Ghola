...
6. **Generate replies.** After SWEs return, write each reply using `parameters.replyInstruction`. 1-2 sentences. Never include severity/rating/SWE attribution.
   ...
   **CodeRabbit persona overlay.** If the parent comment's author display name contains "coderabbit" (case-insensitive), prepend the `parameters.coderabbitReplyPersona` instruction onto the `parameters.replyInstruction` before generating the reply. The persona shapes voice/tone; the instruction shapes content. If `parameters.coderabbitReplyPersona` is empty, treat CodeRabbit replies the same as any other (no overlay). When the overlay IS applied, apply it at the strength set by `tool.operator-profile`'s `parameters.personaIntensity` (0–10), following that module's intensity rubric: at low intensity the reply carries only a whisper of the persona, at high intensity it comes through full-on. If `tool.operator-profile` is not loaded or `personaIntensity` is unset, use the moderate default (5). Intensity affects VOICE only — it never relaxes the no-severity/no-rating/no-attribution rule or the `ok`-before-post approval gate.
...
- Settings (read from the module's parameters block in the Session Manifest):
  ...
  - `parameters.coderabbitReplyPersona` — optional voice/tone overlay layered onto `parameters.replyInstruction` when the parent comment author is CodeRabbit. If absent from the Session Manifest, the default applies: empty string (no overlay). When it IS set, apply it at the strength given by `tool.operator-profile`'s `parameters.personaIntensity` (0–10) per that module's rubric; if `tool.operator-profile` is not loaded or the intensity is unset, use the moderate default (5). The dial affects reply VOICE only — never the no-severity/no-rating/no-attribution rule or the approval gate.
