...
The WRITE verbs — `reply`, `resolve`, `mark-ready`, `to-draft`, and `create pr` (owned by `integration.atlassian-suite`) — mutate Bitbucket and require the Bitbucket token to carry PR write permission: the granular OAuth scope `write:pullrequest:bitbucket`, OR an App Password with "Pull requests: Write". The READ verbs — `find-pr`, `list-comments`, and pipeline status — need only the corresponding read permission (the `read:pullrequest:bitbucket` scope, or an App Password with "Pull requests: Read"). This permission is a property of the token itself, granted on Bitbucket's side when the token is issued; it is not something this module or the Modules tab can turn on. See the 401/403 note in Failure Handling for what to do when a write verb is refused.
...
- 401 / 403 from the API (`status: 'unauthorized'` with message "401 Unauthorized..." or `status: 'forbidden'`) -> cancel the batch. A **401** means the token is invalid or expired: re-set it via the Atlassian Suite's Set Token button. A **403** on a WRITE verb (`reply`, `resolve`, `mark-ready`, `to-draft`, `create pr`) almost always means the token is valid but lacks PR write permission -> re-issue the Bitbucket App Password with "Pull requests: Write" (or an OAuth token with the `write:pullrequest:bitbucket` scope) and re-set it via the Set Token button; alternatively perform the action in the Bitbucket web UI, which uses the browser session rather than the token. There is NO Modules-tab toggle that grants this scope — the write permission is a property of the token, granted on Bitbucket's side when the token is issued, so do not tell the user to "enable write access in the Modules tab" or invent a permission setting. Do not echo the token, do not suggest the user paste it into chat.
