...
- **Ticket resolution at session start** — branch-to-key derivation, the `^([A-Za-z]+)-([0-9]+)` regex, the uppercased-key composition, the fallback ask when the branch yields no match, and the format validation.
...
`pr-monitor.md`'s "mode.ticket-pr Exception (Autonomous Sentry)" section is **authoritative** for whether this posts without the `requireUserApproval` preview; it carries a narrow waiver covering this one literal string and nothing else. Read the scope there — it is deliberately not restated here, because a second copy would drift from the first.
...
**`integration.bitbucket-pr-comments/pr-monitor.md`, section "mode.ticket-pr Exception (Autonomous Sentry)", is AUTHORITATIVE for every waiver in this mode.** That module owns the gates, so the waiver lives with them. Read that section; what follows is a consistent summary, not a second source of truth. Where this file and that section could be read differently, that section wins.
...
**A thread is resolved when, and only when, the `resolved` field on its comment record is `true`.** That field is Bitbucket's own resolution envelope — `PrComment.resolved` in `src/integration/bitbucket-pr-client.ts`, derived as `raw.resolution != null` — already fetched by `list-comments` and already correct. It is the single check.

**Body text is a HINT and NEVER satisfies the check.** "Resolved", "fixed", "done", "addressed", "handled", a checkmark, a green-check emoji, CodeRabbit's own confirmation language — none of it resolves anything. A thread whose latest text claims resolution while `resolved === false` is **NOT done**.

**Moving on from such a thread is the specific defect this rule exists to prevent.** The sentry's job for one of these is to actually issue `resolve` for it (bot-authored, per the autonomy boundary above) or to report it as **still open** (human-authored). Reading the claim, believing it, and advancing to the next ordinal is the failure — it produces a run that reports zero unresolved comments while the PR still shows them.

**A resolved-count derived from body text is wrong by construction and must never be reported.** The roll-up counts a thread resolved only on the boolean.

**Why this is live right now:** CodeRabbit currently lacks permission to resolve its own threads, so its "resolved" text is never accompanied by an actual resolution — every CodeRabbit thread it claims to have closed sits at `resolved === false` until we close it. **The rule is written against the field, not against CodeRabbit**, so it stays correct if CodeRabbit later gains that permission: on that day its claims would coincide with `resolved === true`, and the check would pass for the right reason rather than by luck. Nothing about the rule needs revisiting when that happens — `resolved` is authoritative because it is Bitbucket's state, not because of any assumption about who can currently set it. The ✅-checkmark heuristic in `pr-monitor.md`'s resolve-eligibility gate is flagged there as unverified against a live payload; under autonomy it may only ever ADD caution (hold a bot thread for a closer look before resolving it) and may never substitute for the boolean.
...
When the sentry runs out of work on a **draft** PR, one question is worth the operator's attention: should the PR be published for human review — and only if it is, should the ticket follow it into a review status. This section carries the second and third places this mode addresses the operator: the publish offer, and — only after a successful `mark ready` — the ticket-transition offer. Everything in it is an **offer routed into an existing gate**, never a new gate, and the ticket half never moves on its own.
...
**Check `integration.bitbucket-pr-comments`'s `parameters.markReadyEnabled` FIRST — it defaults to `false`.** When it is off, do not dangle an offer that the verb will refuse: say in one line that the PR is clean and would be offered, that Mark Ready is disabled in the Modules tab, and name that toggle. Then continue to "The otherwise branch" below.
...
So when the operator confirms and `mark ready` **succeeds**, the sentry **resolves the transition and shows it** — the ticket key, its current status, the destination `to.name`, the transition's own `name` for recognition, and the literal numeric id — and then **waits for the operator to confirm that move specifically.** **The procedure, the transition lookup, and the status-matching rules live in `integration.atlassian-suite`'s Jira transition section, which is AUTHORITATIVE for all of them** — read it and follow it; it is not restated here, because a second copy would drift from the first. Resolving is a read (`get-transitions` is ungated there) and composing the offer is safe; the `POST` is the part that waits.
