...
**Board and column queries are NOT available in this mode, and asking for one is not a queue.** "All my tickets in the Review column", "everything assigned to me", "whatever is in the sprint" cannot be honored: there is no JQL, no issue search, and no board endpoint anywhere in this system. `bb-bridge.mjs get-ticket` fetches exactly one key by key, and `integration.atlassian-suite`'s "Sprint and Board Queries" section is prose over an Atlassian MCP board tool this installation does not configure — it is not the missing capability and must not be treated as one. When the operator asks for a column, **say plainly that the queue has to be an explicit list of keys and ask them for the keys.** Do not approximate the column, do not guess at its contents, and do not offer to go and find out.
