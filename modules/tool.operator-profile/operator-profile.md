...
- **`compactProposalThresholdPct` declares 70 as its default, so the manifest always shows a live number for it** — the operator's stored value, or 70 when they haven't overridden it — exactly as an unset `personaIntensity` renders as 5. Keep the two cases apart: *the rendered default* means the operator accepted 70; *malformed* means the operator's value is unusable.
...
`compactProposalThresholdPct` renders the same way `personaIntensity` does: the composer always shows a live value for it — the operator's stored value, or the declared default of **70** when they haven't overridden it — so proposals are **on** by default in a loaded module the operator has never configured, while an explicit `0` or any malformed value means **off** (see "Value handling" above). Loading this module for the name and persona alone still turns the proposals on; `0` is how the operator keeps one without the other.
