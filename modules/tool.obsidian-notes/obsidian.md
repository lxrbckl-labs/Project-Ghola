...
- `<vault>/Support/<APP>.md` — the **per-app knowledge file** for support mode. One per app that `mode.support` has investigated with `parameters.knowledgeFilePerApp` enabled. Accumulates cross-session findings, known gotchas, and resolution patterns for that app. Only written when support mode is active AND the knowledge-file setting is on.
