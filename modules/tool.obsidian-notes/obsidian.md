...
Vault resolution for the agent is therefore a matter of reading `parameters.vaultPath` and branching on whether it is set:

- **`vaultPath` is set (non-empty).** It names the vault for this session — do not second-guess where it came from. What may still need resolving is the path's **form**, not which vault it points at, because `vaultPath` is a single global string shared by hosts on both sides of the WSL boundary: a value stored by one host is routinely spelled in the other's form (`/mnt/c/...` read on native Windows, `C:\...` read on WSL, MSYS `/c/...` from Git Bash). Resolve it in this order and stop at the first step that yields a path that exists:
  1. **Prefer the boot probe's already-resolved `vault`.** When `tool.session-bootstrap` is loaded, its probe has already performed exactly this resolution and reports the result as the digest's `vault` field — accompanied by `vault_translated` (`windows`/`wsl`) when the stored form did not exist and was rewritten into this platform's form, or by `vault_canonicalized=windows` when the stored form resolved fine but was in MSYS `/c/...` form (which bash accepts and the Read/Write tools reject) and was respelled `C:/...`. Adopt that `vault` as the resolved vault path. Both of those cases are **healthy**: the vault was found, only its spelling changed, so neither is a configuration failure and neither may be reported as one.
  2. **Else use the stored value as-is** when `parameters.vaultPath` exists on disk. This is the normal case and it touches nothing.
  3. **Else translate the stored value into this host's native form, and adopt the translation ONLY IF the translated location exists.** The rule is the one every other surface already applies — `src/session/host-path.ts` (`toNativeHostPath`), `scripts/ghola-boot-probe.sh` (`translate_path`), `scripts/ghola.mjs`, and `src/session/launcher.ts`: read `\` as `/`; on native Windows, `/mnt/<letter>/...` and MSYS `/<letter>/...` become `<LETTER>:/...`; on WSL/Linux, `<letter>:/...` and MSYS `/<letter>/...` become `/mnt/<lowercase letter>/...`. Only **single-character** drive letters translate, so genuine mount points (`/mnt/wsl`, `/mnt/host`, `/mnt/data`) and UNC paths are never mangled, and a drive-relative `C:foo` is refused as ambiguous. **A translation is never adopted unverified** — if the translated location does not exist, it is discarded and the original stands, because a plausible-looking invented path is worse than an honest wrong one.
  4. **Only when neither the stored form nor its translation exists** — the probe reports this as `vault_state=unresolved` — is this a configuration failure. Surface it to the user with a one-line instruction to fix `vaultPath` in the Modules tab (or re-run Detect Vault), and continue the session without notes (do not crash the session). Do not silently assume a vault: an unresolvable path means notes genuinely cannot be read or written this session, and it must not be papered over. Equally, do not claim a ticket HAS no notes on this path — the file was never checked.
...
A "session" here means a TPM-led run. The resolved vault path is held in session memory for the duration. The agent does not scan the disk, does not run `find`, and does not write the resolved path back into the manifest — vault DISCOVERY is entirely host-side (the Detect Vault button), and the agent's job is to read the setting and, at most, re-spell it for the host it is running on. Form translation is **not** discovery: it rewrites the one path the setting already names, and only ever to a location it has confirmed exists. It never searches for a different vault.
...
- **Module enabled, `vaultPath` is set but resolves to nothing** — NEITHER the stored form nor its host-native translation exists on disk, which the boot probe reports as `vault_state=unresolved` (see "Configurable: vault location" for the resolution order): surface the failure with a one-line instruction to fix `vaultPath` (or re-run Detect Vault) and continue the session without notes. The user (or the Detect Vault button) supplied a path, and a path that resolves nowhere is a configuration mistake to flag, not to paper over. This case is reached only AFTER the translation step — a stored path that merely failed the first existence check because it is in the other host's form is a resolved, healthy vault (`vault_translated` / `vault_canonicalized`), NOT this case, and reporting a config error for it is the silent-degradation bug this ordering exists to prevent.
