...
- **Jira — read only, unless `integration.jira-comment-write` is enabled.** Ghola validates the token, then reads ticket summary, status, and description (ADF), plus an issue's comments (author, timestamp, and ADF body). Reading comments does **not** imply writing them. With `integration.jira-comment-write` **disabled** (the default), Jira is strictly read-only and no Jira write is authorized. With it **enabled**, exactly one write is unlocked: **posting a new comment to an existing issue.** Nothing else. Ghola still **never** creates an issue, transitions one, assigns it, edits any field, or edits or deletes any comment — including a comment it posted itself. The capability is TPM-only and gated on the operator approving the exact comment text; see that module for the full contract.
...
| 15 | `POST {jiraBase}/rest/api/3/issue/{key}/comment` with `{ body: <ADF doc> }` | Post a new comment to an issue | WRITE | Jira: classic Atlassian API token — **gated on `integration.jira-comment-write`** |
...
Row 15 is the **only** Jira write in the extension. It is reachable only when the operator has enabled `integration.jira-comment-write`; with that module off, no code path posts and the no-ticketing-mutations hard rule applies unchanged. Even with it on, what is unlocked is comment posting alone — not issue creation, not transitions, not field edits, and not editing or deleting existing comments. The Jira token is a classic full-account token that could technically do all of those; the restriction is Ghola's, enforced by there being no other Jira write path in the code.
