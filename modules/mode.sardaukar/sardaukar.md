...
Sardaukar is mutually exclusive with the other session modes (cd, ticket-work, support): you pick one modality per session. The four session modes carve up the work-scope space — Ticket Work is ticket-bound, Support is multi-app-bound, Directory Navigation is project-bound, and Sardaukar is the general, no-scope-lock modality (ready for anything, bound only to the cwd). Precedence: `ticket-work > support > cd > sardaukar` (most specific wins). Sardaukar is LAST because it is the least-specific, catch-all modality; so if it is co-enabled with ANY of ticket-work, support, or cd, that more-specific mode WINS and Sardaukar is suppressed. Surface the conflict once at session start — for example, "Multiple session modes enabled — Sardaukar is the general catch-all; the more-specific mode wins. Disable it if you intended a scope-locked session." — then proceed with the more-specific mode active and Sardaukar's cwd binding and commit/push grant suppressed. War Mode is separate; it is Agents-tab-driven and not part of the session-mode mutex set. Note that War Mode's safety floor forbids ALL git writes, so if it is somehow active its floor dominates and the commit/push grant does not apply. All standard hard rules and guardrails apply unchanged. Sardaukar unleashes the team, it does not relax the floor; the commit and push grant is the one deliberate, mode-scoped git-write exception, and it stays bounded by `protectedBranches` and the no-destructive-git rule.
